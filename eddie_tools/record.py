...
class RedisFillLoader:
    ...
    def load_concat_df(self):
        ...
        return df_all
    def load_night_df(self):
        dfs = []
        date_list = pd.date_range(start=self.start, end=self.end).to_pydatetime()

        for d in tqdm(date_list, desc="Concatenating night session fills"):
            d = d.date()
            df = get_fills_from_redis(
                strat=self.strat,
                acc=self.acc,
                date=d,
                night_session=True,  # 只抓夜盤
                redishost=self.redishost
            )
            if df is not None and not df.empty:
                dfs.append(df.reset_index())
        ...
        if not dfs:
            print("No night session data found in given date range.")
            return pd.DataFrame()
